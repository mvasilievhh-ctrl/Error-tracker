...
def process_update(update):
    ...
    if not message:
        print(f"[bot] update without message/channel_post: {list(update.keys())}")
        return

    chat = message.get("chat", {})
    text_preview = (message.get("text") or message.get("caption") or "")[:60]
    print(f"[bot] incoming message | chat_id={chat.get('id')} chat_type={chat.get('type')} text_preview={text_preview!r}")

    if CHAT_ID and str(chat.get("id")) != str(CHAT_ID):
        print(f"[bot] ignored — chat_id {chat.get('id')} does not match TELEGRAM_CHAT_ID={CHAT_ID}")
        return  # ignore messages from other chats if CHAT_ID is set

    text = message.get("text") or message.get("caption") or ""
    parsed = parse_message(text)
    if not parsed:
        print("[bot] message did not match expected format — skipped")
        return
...
def poll_loop():
    ...
    print(f"[bot] resuming from offset {offset}")
    ...
    while True:
        try:
            ...
            if updates:
                print(f"[bot] received {len(updates)} update(s)")
